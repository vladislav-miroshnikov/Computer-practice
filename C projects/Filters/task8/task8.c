...
void medianFilter(RGB** ptrRGB, RGB** ptrNewRGB, unsigned int height, unsigned int width)
{
	{
		unsigned int i, j, l;
			...
			for (j = 1; j < width - 1; j++)
			{
				...
				for (l = 0; l < 9; )
				{
					for (int k = -1; k < 2; k++)
					{
						for (int m = -1; m < 2; m++)
						{
							redMedian[l] = ptrRGB[i + k][j + m].rgbRed;
							greenMedian[l] = ptrRGB[i + k][j + m].rgbGreen;
							blueMedian[l] = ptrRGB[i + k][j + m].rgbBlue;
							l = l + 1;
						}
					}
				}
				...
				ptrNewRGB[i][j].rgbRed = (unsigned char)(redMedian[4]);
				ptrNewRGB[i][j].rgbGreen = (unsigned char)(greenMedian[4]);
				ptrNewRGB[i][j].rgbBlue = (unsigned char)(blueMedian[4]);
			}
		}
	}
}
...
int main(int argc, char* argv[])
{
	...
	{
		medianFilter(rgb, newRgb, bmpInfo->biHeight, bmpInfo->biWidth);
	}
	else
	{
